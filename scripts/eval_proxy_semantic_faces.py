# scripts/eval_proxy_semantic_faces.py
#
# Execução:
#   python -m scripts.eval_proxy_semantic_faces
#
# Saídas:
#   results/proxy_semantic_metrics.csv
...
def load_seg_ids_per_face(semantic_bin_path: str) -> np.ndarray:
    """
    semantic.bin no seu caso tem 2 uint16 por primitive/face.
    A COLUNA 1 é o segment_id (0..69 aqui; e bate 100% no range do segmentation).
    """
    ...
    seg_ids = pairs[:, 1].astype(np.int32)  # <- coluna correta
    return seg_ids
...
def main():
    ...
    for scene in scenes:
        ...
        out = {"scene": scene, "n_eval_points": n_valid}
        ...
        for lab, name in [(1, "walk"), (2, "support"), (3, "obstruction")]:
            p = (pred_v == lab)
            gm = (gt_v == lab)
            prec, rec, f1, iou = metrics_binary(p, gm)
            out[f"{name}_prec"] = prec
            out[f"{name}_rec"] = rec
            out[f"{name}_f1"] = f1
            out[f"{name}_iou"] = iou

        out["gt_walk_frac"] = float(np.mean(gt_v == 1))
        out["gt_support_frac"] = float(np.mean(gt_v == 2))
        out["gt_obstruction_frac"] = float(np.mean(gt_v == 3))

        rows.append(out)

    df = pd.DataFrame(rows)
    out_csv = os.path.join(RESULTS_DIR, "proxy_semantic_metrics.csv")
    df.to_csv(out_csv, index=False)

    print(f"\n[OK] Salvo: {out_csv}")
    print("\nMédias (todas as cenas) — ignorando NaN:")
